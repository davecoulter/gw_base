...
class Teglon:
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        parser.add_option('--load_local_file', action="store_true", default=True,
                          help='''Circumvent downloading map and load map from disk (Default = False)''')

        parser.add_option('--skip_swope', action="store_true", default=True,
                          help='''Do not register Swope tiles for this map (Default = False)''')

        parser.add_option('--skip_thacher', action="store_true", default=True,
    ...
    def main(self):
        ...
        if len(map_check) > 0:
            is_error = True
            print("The combination of GWID `%s` and healpix file `%s` already exists in the db. Please choose a unique combination" %
...
if __name__ == "__main__":
    useagestring = """python LoadMap.py [options]
